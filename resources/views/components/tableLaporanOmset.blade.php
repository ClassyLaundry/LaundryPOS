    ...
    <div class="table-responsive my-2 tbody-wrap">
        <table class="table mb-0" id="table-laporan" data-total="{{ number_format($total_omset, 0, ',', '.') }}">
            ...
            <tbody>
                ...
                @endphp
                    @if ($index == 0)
                    <tr><td rowspan="{{ $rowHeight[$index2]->count == 1 ? $rowHeight[$index2]->count : $rowHeight[$index2]->count + 1 }}">{{ date('d-M-Y', strtotime($pembayaran->created_at)) }}</td>
                    @else
                        <tr>
                    @endif
                        ...
                        <td><div class="d-flex justify-content-between"><span>Rp</span><span>{{ number_format($pembayaran->nominal, 0, ',', '.') }}</span></div></td>
                    </tr>
                    @if ($rowHeight[$index2]->count == $index + 1 && $index != 0)
                        <tr class="table-success">
                            <td colspan="3" class="text-center">{{ "Total omset per " . date('d-M-Y', strtotime($rowHeight[$index2]->tanggal)) }}</td>
                            <td>
