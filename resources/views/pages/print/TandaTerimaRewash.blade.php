...
<body class="lh-1 ps-3" style="width: 900px; margin-top: 100px;">
    <div id="data-header">
    ...
    <div id="detail-transaksi">
        ...
        <table style="font-size: 10pt">
            ...
            <tbody>
                <tr>
                    <td class="text-start">{{ $data->jenis_item->nama }}</td>
                    <td class="text-center">{{ $data->item->qty }}</td>
                    <td class="text-center">{{ $data->item->satuan_unit }}</td>
                    <td class="text-center">{{ $data->item->bobot_bucket }}</td>
                    <td class="text-center">{{ $data->item->total_bobot }}</td>
                    <td class="text-start">
                        @foreach ($data->item->item_notes as $item_note)
                            @if ($loop->index == 0)
                                {{ $item_note->catatan }}
                            @else
                                {{ ', ' . $item_note->catatan }}
        ...
        <table style="font-size: 10pt">
            ...
            <tbody>
                <tr>
                    ...
                    <td class="text-center">{{ $data->item->qty }}</td>
                    <td class="text-center">{{ $data->item->satuan_unit }}</td>
                    <td class="text-center">{{ $data->item->diskon_jenis_item }}</td>
                    <td class="text-center">{{ number_format($data->item->total_premium, 0, ',', '.') }}</td>
                    <td class="text-start">
                        @foreach ($data->item->item_notes as $item_note)
                            @if ($loop->index == 0)
                                {{ $item_note->catatan }}
        ...
        <div class="d-flex">
            <p>CATATAN:&nbsp;</p>
            <p>{{ "REWASH - " . $data->rewash->jenis_rewash . " || " }} @isset($data->catatan) {{ $data->catatan }} || @endisset {{ $data->transaksi->catatan }}</p>
        </div>
        <hr>
        <div class="row">
